      ...
      cell: ({ row }) => {
        return (
          <div className="flex flex-row gap-1 items-center justify-start">
            ...
            <Button size="xs" variant="destructive" onClick={() => onDeleteRow(row.original.id)}>
              Elimina
            </Button>
          </div>
        );
  ...
  const getStickyLeftPosition = (columnId, columnIndex) => {
    ...
    for (let i = 0; i < columnIndex; i++) {
      ...
      if (stickyColumns.includes(colId)) {
        ...
        if (colId === 'orderNumber') leftPosition += 71; // Numero ODP
        else if (colId === 'code') leftPosition += 80; // Codice
